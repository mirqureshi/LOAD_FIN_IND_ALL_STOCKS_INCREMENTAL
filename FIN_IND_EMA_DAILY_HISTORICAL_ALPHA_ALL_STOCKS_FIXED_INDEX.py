...
print("Historical DAILY EMA from Alpha Vantage started...", flush=True)
...
def fetch_alpha_vantage_ema(session, symbol):
    url = (
    ...
    if key not in data:
        ...
        print(f"{symbol}: no EMA data. Message: {message}", flush=True)
        return []

    rows = []
    for date_str, item in data.get(key, {}).items():
        d = parse_date(date_str)
        ema = parse_float(item.get("EMA"))
        if d is not None and ema is not None:
            rows.append((symbol, INTERVAL, TIME_PERIOD, SERIES_TYPE, d, round(ema, 4)))
    return rows
...
def main():
    ...
    try:
        ...
        print("=" * 70, flush=True)
        print("Historical DAILY EMA from Alpha Vantage", flush=True)
        ...
        print("All non-empty tickers are included. No TRUNCATE. No DROP.", flush=True)
        print("=" * 70, flush=True)
        ...
        for i in range(0, len(symbols), BATCH_SIZE):
            ...
            for j, symbol in enumerate(batch, start=1):
                rows = fetch_alpha_vantage_ema(session, symbol)
                ...
                total_upserted += n
                print(f"{symbol}: batch {batch_number}.{j} ema_rows_upserted={n}", flush=True)
        ...
        print("\n" + "=" * 70, flush=True)
        print("Finished historical DAILY EMA load.", flush=True)
        print(f"Total symbols processed: {len(symbols):,}", flush=True)
        print(f"Total EMA rows upserted: {total_upserted:,}", flush=True)
