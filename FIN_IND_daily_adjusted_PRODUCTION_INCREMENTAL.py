...
# This is where we get the cutoff date:
# SELECT MAX(price_date) FROM FIN_IND.daily_adjusted_data;
MAX_DATE_SCHEMA = "FIN_IND"
MAX_DATE_TABLE = "daily_adjusted_data"

# IMPORTANT:
# Target table is now the original production table.
# New rows after MAX(price_date) will be appended/upserted here.
TARGET_SCHEMA = "FIN_IND"
TARGET_TABLE = "daily_adjusted_data"

# Alpha Vantage output size.
# compact is best for incremental because we only need recent rows.
OUTPUTSIZE = "compact"

# Production table should already exist.
# These are set to False so the script does not try to recreate/index the table.
# This script NEVER truncates or deletes data.
...
# Leave blank to process all tickers.
# Example to resume after a ticker: START_AFTER_TICKER = "IBM"
START_AFTER_TICKER = ""
...
# This is disabled by default for production.
...
def create_target_table_and_indexes(cursor):
    ...
    if CREATE_UNIQUE_INDEX_IF_MISSING:
        ...
        cursor.execute(f"""
            CREATE UNIQUE INDEX IF NOT EXISTS ux_{table}_symbol_price_date
            ON {schema}.{table} (symbol, price_date);
        """)
    ...
    cursor.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{table}_price_date
        ON {schema}.{table} (price_date);
    """)

    cursor.execute(f"""
        CREATE INDEX IF NOT EXISTS idx_{table}_symbol_price_date
        ON {schema}.{table} (symbol, price_date);
    """)


# ============================================================
# Get cutoff max date from FIN_IND.daily_adjusted_data
# ============================================================

def fetch_max_date_from_daily_adjusted_data(cursor):
    schema = safe_identifier(MAX_DATE_SCHEMA)
    table = safe_identifier(MAX_DATE_TABLE)

    print(f"Fetching MAX(price_date) from {schema}.{table}...", flush=True)
    ...
    print(f"MAX(price_date) from {schema}.{table}: {max_date}", flush=True)

    if max_date is None:
        raise SystemExit(
            f"ERROR: MAX(price_date) from {schema}.{table} is None. "
            "Stopping so the script does not accidentally reload full history."
        )
...
# ============================================================
# Fetch Alpha Vantage data for one symbol and insert only newer dates
# ============================================================

def fetch_and_store_daily_adjusted(conn, cursor, session, symbol, max_date_from_daily_table):
    schema = safe_identifier(TARGET_SCHEMA)
    table = safe_identifier(TARGET_TABLE)
    ...
    if "Time Series (Daily)" not in data:
        ...
        print(f"{symbol}: No daily adjusted data returned. Message: {message}", flush=True)
        return 0, 0
    ...
    for date_str, values in data.get("Time Series (Daily)", {}).items():
        ...
        # IMPORTANT:
        # Alpha Vantage does not accept a from_date/start_date parameter
        # for TIME_SERIES_DAILY_ADJUSTED.
        #
        # We fetch compact data, then filter in Python.
        #
        # Example:
        # If FIN_IND.daily_adjusted_data max date is 2026-06-10,
        # this inserts only rows where price_date > 2026-06-10.
        if price_date <= max_date_from_daily_table:
            continue
        ...
        rows.append((
            ...
            to_float(values.get("5. adjusted close")),
            to_bigint(values.get("6. volume")),
            to_float(values.get("7. dividend amount")),
            to_float(values.get("8. split coefficient")),
        ))
...
def main():
    ...
    try:
        ...
        print(f"MAX date source table: {MAX_DATE_SCHEMA}.{MAX_DATE_TABLE}", flush=True)
        ...
        # Step 1: Get max date from the ORIGINAL production table.
        max_date_from_daily_table = fetch_max_date_from_daily_adjusted_data(cursor)
        ...
        print(f"Using MAX date from original table {MAX_DATE_SCHEMA}.{MAX_DATE_TABLE}: {max_date_from_daily_table}", flush=True)
        print(f"Will append/upsert only rows where price_date > {max_date_from_daily_table}", flush=True)
        ...
        for i in range(0, len(stock_symbols), BATCH_SIZE):
            ...
            for j, symbol in enumerate(batch_symbols, start=1):
                n, api_rows_seen = fetch_and_store_daily_adjusted(
                    conn=conn,
                    cursor=cursor,
                    session=session,
                    symbol=symbol,
                    max_date_from_daily_table=max_date_from_daily_table
                )
                ...
                print(
                    ...
                    f"max_date_from_original_daily_adjusted_data={max_date_from_daily_table} "
        ...
        print("\n" + "=" * 70, flush=True)
        print("Finished incremental update into original daily_adjusted_data table.", flush=True)
        print(f"Total symbols processed: {len(stock_symbols):,}", flush=True)
        print(f"MAX date source table: {MAX_DATE_SCHEMA}.{MAX_DATE_TABLE}", flush=True)
        print(f"MAX date used: {max_date_from_daily_table}", flush=True)
