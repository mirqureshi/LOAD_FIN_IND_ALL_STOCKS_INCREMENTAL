...
def max_date(cur):
    s=safe_identifier(TARGET_SCHEMA); t=safe_identifier(TARGET_TABLE)
    cur.execute(f"SELECT MAX(date) FROM {s}.{t} WHERE interval=%s AND time_period=%s AND fast_period=%s AND slow_period=%s AND series_type=%s;",(INTERVAL,TIME_PERIOD,FAST_PERIOD,SLOW_PERIOD,SERIES_TYPE))
    m=cur.fetchone()[0]
