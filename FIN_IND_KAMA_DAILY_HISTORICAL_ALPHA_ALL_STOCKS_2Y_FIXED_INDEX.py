...
def setup(cur):
    ...
    cur.execute(f"SELECT COUNT(*) FROM (SELECT symbol, interval, time_period, fast_period, slow_period, series_type, date FROM {s}.{t} GROUP BY symbol, interval, time_period, fast_period, slow_period, series_type, date HAVING COUNT(*)>1) d;")
    if cur.fetchone()[0]:
        cur.execute(f"DELETE FROM {s}.{t} a USING {s}.{t} b WHERE a.id>b.id AND a.symbol=b.symbol AND a.interval=b.interval AND a.time_period=b.time_period AND a.fast_period=b.fast_period AND a.slow_period=b.slow_period AND a.series_type=b.series_type AND a.date=b.date;")
...
def fetch_indicator(session, symbol):
    url=("https://www.alphavantage.co/query" + f"?function=KAMA&symbol={symbol}&interval={INTERVAL}&time_period={TIME_PERIOD}&series_type={AV_SERIES_TYPE}&apikey={API_KEY}")
    try:
    ...
    if not ta:
        ...
        print(f'{symbol}: no KAMA data. Message: {msg}', flush=True); return []
    rows=[]
...
def main():
    ...
    try:
        ...
        print(f'Historical DAILY KAMA Alpha Vantage 2-year load. Start date={{HISTORY_START_DATE}} symbols={{len(symbols):,}}', flush=True)
        ...
        for i in range(0,len(symbols),BATCH_SIZE):
            ...
            for j,sym in enumerate(batch,1):
                ...
                if n==-1: n=0
                total+=n; print(f'{sym}: batch {bn}.{j} kama_rows_upserted={{n}}', flush=True)
        ...
        print(f'Finished KAMA historical load. Total rows={{total:,}} elapsed={{perf_counter()-started:.2f}}s', flush=True)
    finally:
