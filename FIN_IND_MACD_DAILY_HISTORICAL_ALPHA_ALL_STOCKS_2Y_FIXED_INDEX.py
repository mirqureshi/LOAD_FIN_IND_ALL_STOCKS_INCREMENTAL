...
def setup(cur):
    ...
    cur.execute(f"SELECT COUNT(*) FROM (SELECT symbol, interval, fast_period, slow_period, signal_period, series_type, date FROM {s}.{t} GROUP BY symbol, interval, fast_period, slow_period, signal_period, series_type, date HAVING COUNT(*)>1) d;")
    if cur.fetchone()[0]:
        cur.execute(f"DELETE FROM {s}.{t} a USING {s}.{t} b WHERE a.id>b.id AND a.symbol=b.symbol AND a.interval=b.interval AND a.fast_period=b.fast_period AND a.slow_period=b.slow_period AND a.signal_period=b.signal_period AND a.series_type=b.series_type AND a.date=b.date;")
...
def fetch_indicator(session, symbol):
    url=("https://www.alphavantage.co/query" + f"?function=MACD&symbol={symbol}&interval={INTERVAL}&series_type={AV_SERIES_TYPE}&fastperiod={FAST_PERIOD}&slowperiod={SLOW_PERIOD}&signalperiod={SIGNAL_PERIOD}&apikey={API_KEY}")
    try:
    ...
    if not ta:
        ...
        print(f'{symbol}: no MACD data. Message: {msg}', flush=True); return []
    rows=[]
    for ds,row in ta.items():
        d=parse_date(ds)
        if d is None or d < HISTORY_START_DATE: continue
        macd=parse_float(item.get("MACD")); sig=parse_float(item.get("MACD_Signal")); hist=parse_float(item.get("MACD_Hist"))
        if macd is None or sig is None or hist is None: continue
        rows.append((symbol, INTERVAL, FAST_PERIOD, SLOW_PERIOD, SIGNAL_PERIOD, SERIES_TYPE, d, round(macd,4), round(sig,4), round(hist,4)))
    return rows
...
def main():
    ...
    try:
        ...
        print(f'Historical DAILY MACD Alpha Vantage 2-year load. Start date={{HISTORY_START_DATE}} symbols={{len(symbols):,}}', flush=True)
        ...
        for i in range(0,len(symbols),BATCH_SIZE):
            ...
            for j,sym in enumerate(batch,1):
                ...
                if n==-1: n=0
                total+=n; print(f'{sym}: batch {bn}.{j} macd_rows_upserted={{n}}', flush=True)
        ...
        print(f'Finished MACD historical load. Total rows={{total:,}} elapsed={{perf_counter()-started:.2f}}s', flush=True)
    finally:
