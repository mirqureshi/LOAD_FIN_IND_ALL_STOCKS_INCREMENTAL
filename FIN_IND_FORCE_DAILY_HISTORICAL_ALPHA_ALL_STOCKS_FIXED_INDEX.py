...
print("Historical DAILY FORCE from Alpha Vantage started...", flush=True)
...
def fetch_alpha_vantage_force(session, symbol):
    url = (
    ...
    try:
        ...
        return []
    key = "Technical Analysis: FORCE"
    if key not in data:
        message = data.get("Note") or data.get("Information") or data.get("Error Message") or str(data)[:400]
        print(f"{symbol}: no FORCE data. Message: {message}", flush=True)
        return []
    rows = []
    for date_str, item in data.get(key, {}).items():
        d = parse_date(date_str)
        force_value = parse_float(item.get("Force Index"))
        if d is None or force_value is None:
            continue
        rows.append((symbol, INTERVAL, TIME_PERIOD, SERIES_TYPE, d, round(force_value, 4)))
    return rows
...
def main():
    ...
    try:
        ...
        print("=" * 70, flush=True)
        print("Historical DAILY FORCE from Alpha Vantage", flush=True)
        ...
        print(f"Series type stored: {SERIES_TYPE}", flush=True)
        ...
        for i in range(0, len(symbols), BATCH_SIZE):
            ...
            for j, symbol in enumerate(batch, start=1):
                rows = fetch_alpha_vantage_force(session, symbol)
                ...
                if n == -1:
                    n = 0
                total_upserted += n
                print(f"{symbol}: batch {batch_number}.{j} force_rows_upserted={n}", flush=True)
        ...
        print("\n" + "=" * 70, flush=True)
        print("Finished historical DAILY FORCE load.", flush=True)
        print(f"Total symbols processed: {len(symbols):,}", flush=True)
        print(f"Total FORCE rows upserted: {total_upserted:,}", flush=True)
