...
def ensure_unique_index_for_upsert(cursor):
    ...
    print("Checking duplicate EMA rows before creating unique index...", flush=True)
    cursor.execute(f"""
        SELECT COUNT(*)
        FROM (
    ...
    if duplicate_groups and duplicate_groups > 0:
        print(f"Found {duplicate_groups:,} duplicate EMA key groups. Removing duplicates and keeping lowest id...", flush=True)
        cursor.execute(f"""
        ...
        print("No duplicate EMA key groups found.", flush=True)
...
def fetch_all_stock_symbols_from_master(cursor):
    ...
    symbols = [r[0] for r in cursor.fetchall() if r[0]]
    print(f"Fetched {len(symbols):,} symbols from {schema}.{table}.", flush=True)
    return symbols

def upsert_ema_rows(conn, cursor, rows):
    if not rows:
        return 0
    ...
    sql = f"""
        INSERT INTO {schema}.{table} (
            symbol, interval, time_period, series_type, date, ema
        )
        VALUES %s
...
print("Incremental DAILY EMA from daily_adjusted_data started...", flush=True)
...
def fetch_max_ema_date(cursor):
    ...
    print(f"MAX EMA date from {schema}.{table} for {INTERVAL}/{TIME_PERIOD}/{SERIES_TYPE}: {max_date}", flush=True)

    if max_date is None:
        raise SystemExit(
            f"No existing {INTERVAL} EMA found for series_type={SERIES_TYPE}. "
            "Run the historical DAILY EMA load first."
        )
    return max_date

def fetch_daily_price_batch(conn, symbols, max_ema_date):
    if not symbols:
    ...
    # EMA is recursive, so give enough warmup data before max date.
    ...
    with conn.cursor() as cur:
        cur.execute(query, (symbols, max_ema_date, lookback_days))
        rows = cur.fetchall()
...
def calculate_ema_like_api(df, price_col, period):
    """
    Same logic from your incremental file:
    1. K = 2 / (period + 1)
    2. First EMA = SMA of first period prices
    3. EMA = EMA_previous + K * (price - EMA_previous)
    """
    prices = df[price_col].astype(float).reset_index(drop=True)
    ema = [None] * len(prices)
    ...
    k = 2 / (period + 1)
    first_ema = prices.iloc[:period].mean()
    ema[period - 1] = first_ema

    for i in range(period, len(prices)):
        ema[i] = ema[i - 1] + k * (prices.iloc[i] - ema[i - 1])

    return pd.Series(ema)

def calculate_daily_ema_for_symbol(df):
    df = df.copy()
    df = df.sort_values("date").reset_index(drop=True)

    if len(df) < TIME_PERIOD:
        return pd.DataFrame(columns=["symbol", "date", "ema"])

    df["ema"] = calculate_ema_like_api(df, "price", TIME_PERIOD)
    return df.dropna(subset=["ema"])[["symbol", "date", "ema"]]

def build_ema_rows_for_batch(price_df, max_ema_date):
    if price_df.empty:
        return []
    ...
    for symbol, symbol_prices in price_df.groupby("symbol"):
        ema_df = calculate_daily_ema_for_symbol(symbol_prices)

        for _, row in ema_df.iterrows():
            if row["date"] <= max_ema_date:
                continue

            rows.append((symbol, INTERVAL, TIME_PERIOD, SERIES_TYPE, row["date"], round(float(row["ema"]), 4)))
...
def main():
    ...
    try:
        ...
        max_ema_date = fetch_max_ema_date(cursor)
        symbols = fetch_all_stock_symbols_from_master(cursor)

        print("=" * 70, flush=True)
        print("Incremental DAILY EMA from daily_adjusted_data", flush=True)
        ...
        print(f"MAX EMA date used: {max_ema_date}", flush=True)
        print(f"Total symbols: {len(symbols):,}", flush=True)
        print("No Alpha Vantage calls. No weekly conversion. No TRUNCATE. No DROP.", flush=True)
        print("=" * 70, flush=True)
        ...
        for i in range(0, len(symbols), BATCH_SIZE):
            ...
            price_df = fetch_daily_price_batch(conn, batch, max_ema_date)
            rows = build_ema_rows_for_batch(price_df, max_ema_date)
            n = upsert_ema_rows(conn, cursor, rows)
            ...
            print(
                ...
                f"new_ema_rows_upserted={n:,} elapsed={perf_counter() - batch_start:.2f}s",
                flush=True
            )
        ...
        print("\n" + "=" * 70, flush=True)
        print("Finished incremental DAILY EMA load.", flush=True)
        print(f"Total symbols processed: {len(symbols):,}", flush=True)
        print(f"Total new EMA rows upserted: {total_upserted:,}", flush=True)
