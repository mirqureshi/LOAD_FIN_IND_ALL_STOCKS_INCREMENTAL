...
def max_date(cur):
    s=safe_identifier(TARGET_SCHEMA); t=safe_identifier(TARGET_TABLE)
    cur.execute(f"SELECT MAX(date) FROM {s}.{t} ;",(INTERVAL,TIME_PERIOD))
    m=cur.fetchone()[0]
    if m is None: raise SystemExit('No existing MFI data found. Run historical load first.')
    return m
def fetch_source(conn,symbols,m,date_col):
    if not symbols: return pd.DataFrame(columns=['symbol', 'date', 'high', 'low', 'close', 'volume'])
    ss=safe_identifier(SOURCE_SCHEMA); st=safe_identifier(SOURCE_TABLE); dc=safe_identifier(date_col)
    q=f"SELECT symbol, {dc}::date AS date, high::numeric AS high, low::numeric AS low, adjusted_close::numeric AS close, volume::numeric AS volume FROM {ss}.{st} WHERE symbol=ANY(%s) AND {dc}::date >= (%s::date - %s::int) AND {dc} IS NOT NULL AND high IS NOT NULL AND low IS NOT NULL AND adjusted_close IS NOT NULL AND volume IS NOT NULL ORDER BY symbol, {dc} ASC;"
    with conn.cursor() as c:
        c.execute(q,(symbols,m,LOOKBACK_DAYS)); rows=c.fetchall()
    return pd.DataFrame(rows,columns=['symbol', 'date', 'high', 'low', 'close', 'volume'])
def calc_one(df):
    df=df.copy().sort_values('date').reset_index(drop=True)
    if len(df)<=TIME_PERIOD: return pd.DataFrame(columns=['symbol','date','mfi'])
    df[['high','low','close','volume']]=df[['high','low','close','volume']].astype(float)
    df['typical_price']=(df['high']+df['low']+df['close'])/3
    df['money_flow']=df['typical_price']*df['volume']
    prev=df['typical_price'].shift(1)
    df['positive_flow']=0.0; df['negative_flow']=0.0
    df.loc[df['typical_price']>prev,'positive_flow']=df['money_flow']
    df.loc[df['typical_price']<prev,'negative_flow']=df['money_flow']
    df['positive_sum']=df['positive_flow'].rolling(TIME_PERIOD).sum()
    df['negative_sum']=df['negative_flow'].rolling(TIME_PERIOD).sum()
    df['money_ratio']=df['positive_sum']/df['negative_sum']
    df['mfi']=100-(100/(1+df['money_ratio']))
    df.loc[df['negative_sum']==0,'mfi']=100
    return df.dropna(subset=['mfi'])[['symbol','date','mfi']]
def build_rows(source_df,m):
    rows=[]
    for symbol,g in source_df.groupby('symbol'):
        for _,r in calc_one(g).iterrows():
            if r['date']<=m: continue
            rows.append((symbol,INTERVAL,TIME_PERIOD,r['date'],round(float(r['mfi']),4)))
    return rows
...
def main():
    ...
    try:
        ...
        print(f'Incremental DAILY MFI. max_date={{m}} symbols={{len(symbols):,}}',flush=True)
        total=0
        ...
        print(f'Finished MFI incremental load. total_new_rows={{total:,}} elapsed={{perf_counter()-started:.2f}}s',flush=True)
    finally:
